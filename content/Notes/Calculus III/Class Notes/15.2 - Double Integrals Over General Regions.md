...
>[!Example]
> Find the volume of the solid that lies under the paraboloid $z = x^2 + y^2$ and above the region $\partial$ in the xy-plane, bounded by the line $y=2x$ and $y=x^2$

>[!Success] Solution
> **Sketch the region $\partial$**
...
**Now sketch the integration region $\partial$ in the xy-plane:**
...
$$\partial = \left\{  (x,y) \ : \ 0 \leq x \leq 1 \ , \ \frac{x}{2} \leq y \leq 1 - \frac{x}{2} \right\}$$

(we got the bounds on y because $x=2y \rightarrow y = \frac{x}{2}, \ \ x +2y=2 \rightarrow y=1 - \frac{x}{2}$)

The volume of the tetrahedron is the volume above $\partial$ and below the plane $x + 2y + z = 2$, which we can express as $z = 2-x-2y$.

**The double integral is:**

$$\int \int_{\partial} f(x,y) dA = \int_{0}^1 \int_{\frac{x}{2}}^{1-\left( \frac{x}{2} \right)}2-x-2y \ dy \ dx = \frac{1}{3}$$
...
We can work backwards to find limits of integration for the order $dx \ dy$. By reading the original integral, we see that $\partial$ is the region where $y$ "runs" from $y=x$ to $y=1$, and $x$ "runs" from $x=0$ to $x=1$

>[!Info] Sketch
> ![[Screenshot 2024-10-07 at 12.24.45 PM.png]]

**Integration Region $\partial$** ( type 1):
...
From the **Sketch**, we see that $\partial$ is treated as a Type 1 region in the original integral. If we treat $\partial$ as a Type 2 region, the integral becomes:
...
$$\int \int_{\partial} 1 dA = A(\partial)$$

Where $A(\partial)$ means the area of region $\partial$. This is because the integral represents the volume of the solid below the plane $z=1$ and above the region $\partial$, which is merely a cylinder:
